...
class CarouselItem extends Component {
  ...
  translateItem(index, activeIndex, oldIndex) {
    ...
    if (component.iscard) {
      ...
      this.setState({
        ...
        translate: self.calculateTranslate(index, activeIndex, parentWidth, inStage),
        scale: index === activeIndex ? 1 : self.CARD_SCALE
      })
    } else {
      this.setState({
        active: index === activeIndex,
        translate: parentWidth * (index - activeIndex),
      })
    }
    this.setState({
      ready: true
  ...
  render() {
    ...
    return ready && (
      <div className={classnames('rain-carousel__item', {
        'is-active': active,
        'rain-carousel__item-card': iscard,
        'is-in-stage': inStage,
